...
const EXCLUDED_DIRS = new Set(['.git', 'node_modules', 'target', 'dist', 'build', 'vendor', '.venv', '__pycache__']);
...
async function astGrepScan({ path: targetPath, pattern, lang }) {
  if (!targetPath || !pattern || !lang) throw new Error('path, pattern, and lang are required');

  const result = await runCommand('ast-grep', ['run', '--pattern', pattern, '--lang', lang, '--json=stream', targetPath], {
    timeoutMs: 120_000,
  });
  if (result.notFound) {
    return { tool: 'ast-grep', available: false, message: notFoundMessage('ast-grep', 'brew install ast-grep, or cargo install ast-grep') };
  }
  ...
  return { tool: 'ast-grep', available: true, match_count: matches.length, matches };
}
...
async function smtCheckReachability({ smt2_script: smt2Script }) {
  ...
  const script = smt2Script.includes('(check-sat)') ? smt2Script : `${smt2Script}\n(check-sat)\n`;
  const result = await runCommand('z3', ['-in'], { input: script, timeoutMs: 30_000 });
  if (result.notFound) {
    return { tool: 'z3', available: false, message: notFoundMessage('z3', 'brew install z3, or apt-get install z3') };
  }
...
createServer({
  ...
  tools: [
    {
      name: 'ast_grep_scan',
      description: 'Structural AST pattern search via ast-grep (tree-sitter backed). Use for precise call-graph/API-usage queries beyond plain regex.',
      inputSchema: {
        type: 'object',
        properties: {
          path: { type: 'string', description: 'File or directory to search.' },
          pattern: { type: 'string', description: 'ast-grep pattern, e.g. "exec($CMD)".' },
          lang: { type: 'string', description: 'Language id, e.g. javascript, typescript, python, go, java, rust.' },
        },
      ...
      inputSchema: {
        ...
        properties: {
          ...
          languages: {
            ...
            description: 'Restrict to these language rule sets. Defaults to all.',
          },
        },
        required: ['path'],
      },
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          smt2_script: {
            type: 'string',
            description: 'SMT-LIB2 script (declare-const/assert statements). A trailing (check-sat) is added automatically if missing.',
          },
        },
